...
public record GameDeck(HeroType heroType, List<Card> cards) {
    ...
    static final UntargetedSpell CLEAVE_SPELL = new UntargetedSpell(s -> s.opponent().getTerritory().getMinionCount() >= 2, s -> s.opponent().getTerritory().getRandomMinions(2).forEach(m -> m.takeDamage(2)));
    static final Card CLEAVE = new SpellCard("Cleave", 2, "deal 2 damage to 2 random enemy minions", CLEAVE_SPELL);


    static final TargetedSpell CHARGE_SPELL = new TargetedSpell(ALLIED_MINION, (t, s) -> t.enhance(stats -> stats.addProperty(MinionProperty.CHARGE).changeAttack(2)));
    static final Card CHARGE = new SpellCard("Charge", 3, "give a friendly minion +2 attack and Charge", CHARGE_SPELL);

    static final TargetedSpell EXECUTE_SPELL = new TargetedSpell(new TargetClassification(TargetType.MINION, SideType.ENEMY, (t, s) -> t.getHealth() < t.getMaxHealth()), (t, s) -> t.destroy());
    ...
    static final UntargetedSpell HEROIC_STRIKE_SPELL = new UntargetedSpell(NO_PRECONDITIONS_FOR_UNTARGETED, s -> s.own().getHero().enhance(stats -> stats.changeAttack(4), 1));
    ...
    static final UntargetedSpell WHIRLWIND_SPELL = new UntargetedSpell(NO_PRECONDITIONS_FOR_UNTARGETED, s -> {
    ...
    static final List<Card> basicWarriorCards = List.of(ARCANITE_REAPER, ARCANITE_REAPER, CLEAVE, CLEAVE, CHARGE, CHARGE, EXECUTE, EXECUTE, FIERY_WAR_AXE, FIERY_WAR_AXE, FROSTWOLF_GRUNT, FROSTWOLF_GRUNT, FROSTWOLF_WARLORD, FROSTWOLF_WARLORD, GURUBASHI_BERSERKER, GURUBASHI_BERSERKER, HEROIC_STRIKE, HEROIC_STRIKE, KOK_KRON_ELITE, KOK_KRON_ELITE, RAID_LEADER, RAID_LEADER, SEN_JIN_SHIELDMASTA, SEN_JIN_SHIELDMASTA, SHIELD_BLOCK, SHIELD_BLOCK, WARSONG_COMMANDER, WARSONG_COMMANDER, WHIRLWIND, WHIRLWIND);
    ...
    static final UntargetedSpell ARCANE_EXPLOSION_SPELL = new UntargetedSpell(NO_PRECONDITIONS_FOR_UNTARGETED, s -> s.opponent().getTerritory().getMinions().forEach(m -> m.takeDamage(1)));
    ...
    static final UntargetedSpell ARCANE_INTELLECT_SPELL = new UntargetedSpell(NO_PRECONDITIONS_FOR_UNTARGETED, s -> {
        s.own().drawCard();
        s.own().drawCard();
    });
    ...
    static final UntargetedSpell ARCANE_MISSILES_SPELL = new UntargetedSpell(NO_PRECONDITIONS_FOR_UNTARGETED, s -> {
        for (int i = 0; i < 3; i++) {
    ...
    static Minion mirrorImageMinion() {
        return new Minion("Mirror image", 0, 2, List.of(Enhancement.TAUNT));
    }
    ...
    static final UntargetedSpell MIRROR_IMAGE_SPELL = new UntargetedSpell(NO_PRECONDITIONS_FOR_UNTARGETED, s -> {
        ...
        for (int minionCount = 0; minionCount < 2; minionCount++) {
            if (ownTerritory.canAddMinion()) ownTerritory.addMinion(mirrorImageMinion());
        }
    });
    ...
    static final List<Card> basicMageCards = List.of(ARCANE_EXPLOSION, ARCANE_EXPLOSION, ARCANE_INTELLECT, ARCANE_INTELLECT, ARCANE_MISSILES, ARCANE_MISSILES, ARCHMAGE, ARCHMAGE, DARKSCALE_HEALER, DARKSCALE_HEALER, FIREBALL, FIREBALL, FLAMESTRIKE, FLAMESTRIKE, FROST_NOVA, FROST_NOVA, FROSTBOLT, FROSTBOLT, KOBOLD_GEOMANCER, KOBOLD_GEOMANCER, MAGMA_RAGER, MAGMA_RAGER, MIRROR_IMAGE, MIRROR_IMAGE, POLYMORPH, POLYMORPH, VOODOO_DOCTOR, VOODOO_DOCTOR, WATER_ELEMENTAL, WATER_ELEMENTAL);
